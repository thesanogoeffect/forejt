...
PRESEASON = datetime.datetime.now() < datetime.datetime(2025, 9, 2)
logging.basicConfig(
    level=logging.INFO,
...
def all_dfs():
    try:
        dfs = pd.read_html(
            "https://www.psmf.cz/souteze/2025-hanspaulska-liga-podzim/6-b/tymy/forejt-fc/"
        )
        return dfs
...
def matches_df(all_dfs, pitches_df):
    ...
    matches_df["Domácí - Hosté"] = matches_df["Domácí - Hosté"].str.replace(
        "  ", " vs. "
    )
    matches_df["Zkratka hřiště base"] = matches_df["Hřiště"].str.extract(
        "(^[A-Z]+)", expand=True
    )
    ...
    matches_df["Datum"] = (
        matches_df["Datum"]
        .str.replace("[A-Za-zÚČá]+", "", regex=True)
        .str.replace("\xa0", "")
    )
...
def results_df(all_dfs, pitches_df):
    ...
    results_df["Domácí - Hosté"] = results_df["Domácí - Hosté"].str.replace(
        "  ", " vs. "
    )
    results_df["Zkratka hřiště base"] = results_df["Hřiště"].str.extract(
        "(^[A-Z]+)", expand=True
    )
